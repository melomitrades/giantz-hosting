...
// â”€â”€ Joiner Modal â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function JoinerModal({ initial, isNew, onClose }: { initial: User; isNew: boolean; onClose: () => void }) {
  ...
  return (
    <Modal title={isNew ? "Add Joiner" : `Edit â€” ${initial.name}`} onClose={onClose} width={420}>
      <form onSubmit={submit}>
        <FormField label="Username / Display Name">
          <input required value={form.name} onChange={(e) => setForm((p) => ({ ...p, name: e.target.value }))} placeholder="e.g. ì†Œìœ¨ (Soyul)" autoFocus />
        </FormField>
...
// â”€â”€ Group Modal â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function GroupModal({ initial, isNew, onClose }: { initial: KnownGroup; isNew: boolean; onClose: () => void }) {
  ...
  const submit = (e: React.FormEvent) => {
    e.preventDefault();
    // Send clean object â€” API expects { id, name, members, fixedJoiners }
    const payload = {
      id: group.id,
      name: group.name,
  ...
  return (
    <Modal title={isNew ? "Add Group" : `Edit â€” ${initial.name}`} onClose={onClose} width={500}>
      <form onSubmit={submit}>
            ...
            {group.members.map((m) => (
              <span key={m.id} style={{ display: "flex", alignItems: "center", gap: 4, padding: "3px 10px", borderRadius: 99, background: "var(--accent-blossom-dim)", color: "var(--accent-blossom)", fontSize: "0.8rem" }}>
                ...
                <button type="button" onClick={() => setGroup((g) => ({ ...g, members: g.members.filter((x) => x.id !== m.id) }))}
                  style={{ background: "none", border: "none", color: "var(--accent-blossom)", cursor: "pointer", fontSize: "0.65rem" }}>âœ•</button>
              </span>
            ))}
    ...
    <div className="fade-in">
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-secondary text-sm mt-1">
            {joiners.length} joiner{joiners.length !== 1 ? "s" : ""} Â· {knownGroups.length} group{knownGroups.length !== 1 ? "s" : ""}
          </p>
        </div>
      ...
      <div style={{ display: "flex", gap: 4, marginBottom: "1.5rem", background: "var(--bg-elevated)", borderRadius: "var(--radius-md)", padding: 4, width: "fit-content" }}>
        {(["joiners", "groups"] as Tab[]).map((t) => (
          <button key={t} onClick={() => setTab(t)}
            ...
            {t === "joiners" ? "ðŸ‘¤ Joiners" : "ðŸŽ¤ Groups"}
          </button>
        ))}
      </div>
      ...
      {tab === "joiners" && (
        <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
                    ...
                    <div>
                      ...
                      {fixedGroups.length > 0 && (
                        <div style={{ display: "flex", gap: 4, marginTop: 4, flexWrap: "wrap" }}>
                          {fixedGroups.map((g) => (
                            <span key={g} style={{ fontSize: "0.68rem", padding: "1px 7px", borderRadius: 99, background: "var(--accent-mint-dim)", color: "var(--accent-mint)" }}>âœ“ Fixed: {g}</span>
                          ))}
                        </div>
                      )}
                    </div>
          ...
          {joiners.length === 0 && <div className="card" style={{ textAlign: "center", padding: "3rem", color: "var(--text-muted)" }}>No joiners yet âœ¦</div>}
        </div>
      )}
      ...
      {tab === "groups" && (
        <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
                  ...
                  <div>
                    ...
                    {fixedNames.length > 0 && (
                      <div style={{ display: "flex", gap: 4, marginTop: 6, flexWrap: "wrap" }}>
                        {fixedNames.map((name) => (
                          <span key={name} style={{ fontSize: "0.68rem", padding: "1px 7px", borderRadius: 99, background: "var(--accent-mint-dim)", color: "var(--accent-mint)" }}>âœ“ Fixed: {name}</span>
                        ))}
                      </div>
                    )}
                  </div>
          ...
          {knownGroups.length === 0 && <div className="card" style={{ textAlign: "center", padding: "3rem", color: "var(--text-muted)" }}>No groups yet âœ¦</div>}
        </div>
      )}
    </div>
  );
