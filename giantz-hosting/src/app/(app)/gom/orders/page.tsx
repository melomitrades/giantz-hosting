...
function GroupManagerModal({ initial, isNew, onClose }: { initial: KnownGroup; isNew: boolean; onClose: () => void }) {
  ...
  const [group, setGroup] = useState<KnownGroup>({ ...initial, members: [...initial.members], fixedJoiners: [...(initial.fixedJoiners ?? [])] });
  ...
  return (
    <Modal title={isNew ? "Add Group" : `Edit — ${initial.name}`} onClose={onClose} width={480}>
        ...
        {joiners.length > 0 && (
          <div style={{ marginTop: "1rem" }}>
            ...
            <div style={{ fontSize: "0.72rem", color: "var(--text-muted)", marginBottom: 8 }}>
              These joiners are automatically added to every new order for this group.
            </div>
...
function ShopOrderModal({ initial, isNew, onClose }: { initial: ShopOrder; isNew: boolean; onClose: () => void }) {
  ...
  // When group changes, auto-add fixed joiners that aren't already in the list
  const handleGroupChange = (groupName: string) => {
    if (!groupName) { setField("group", groupName); return; }
    const kg = knownGroups.find((g) => g.name.toLowerCase() === groupName.toLowerCase());
    const fixedIds = kg?.fixedJoiners ?? [];
    setForm((p) => {
      const existingIds = new Set(p.joiners.map((j) => j.joinerId));
      const toAdd = fixedIds
        .filter((id) => !existingIds.has(id))
        .map((id) => { const u = users.find((u) => u.id === id); return u ? emptyJoiner(u.id, u.name) : null; })
        .filter(Boolean) as JoinerEntry[];
      return { ...p, group: groupName, joiners: [...p.joiners, ...toAdd] };
    });
  };
      ...
      <form onSubmit={submit}>
        ...
        <FormRow cols={2}>
          <FormField label="Group">
            <div style={{ display: "flex", gap: 6 }}>
              <select style={{ flex: 1 }} value={form.group} onChange={(e) => handleGroupChange(e.target.value)} required>
